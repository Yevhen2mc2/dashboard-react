...
function generateTrend(
  ...
  // Generate 11 historical points with random walk
  for (let i = 0; i < 11; i++) {
    const variance = (Math.random() - 0.5) * 2 * variancePercent;
    value = value * (1 - variance / 100);
    trend.push(value);
  }
...
export function generateOverviewData(country: Country): OverviewData {
  ...
  const defaultRate = faker.number.float({
    min: 1.5,
    max: 4.5,
    fractionDigits: 2,
  });
  ...
  return {
    ...
    averageInterestRate,
    defaultRate,
    totalActiveLoansTrend: generateTrend(totalActiveLoans, 5),
    totalLoanAmountTrend: generateTrend(totalLoanAmount, 5),
    averageInterestRateTrend: generateTrend(averageInterestRate, 3),
    defaultRateTrend: generateTrend(defaultRate, 4),
  };
}
